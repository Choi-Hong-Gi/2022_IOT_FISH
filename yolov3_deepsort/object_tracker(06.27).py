        ...
        for track in tracker.tracks:
            ...
            if old_detection != None:
                ...
                for oKey, oValue in old_detection.items():
                    ...
                    if abs(c_curr[0]-oValue[0]) + abs(c_curr[1]-oValue[1]) < 5:
                        ...
                        for p in pattern[-50::5]:
                            ...
                            if pre_p != c_curr:
                                ...
                                if saveTimer >= 10:
                                    ...
                                    date = x.strftime("%x")
                                    time_ = x.strftime("%X")
                                    num_obj = str(max(detected))
                                    
                                    for ids, moving in fish_moving.items():
                                        Mobility.append(round( sum(moving)/100000, 2))

                                    avgMob = str(round( sum(Mobility)/len(Mobility)))
                                    data = (date, time_, num_obj, avgMob)
                                    sql = "INSERT INTO fish_activity (Date, Time, num_obj, mobility) VALUES(%s, %s, %s, %s);"
                                    if Mobility != 0:
                                        try:
