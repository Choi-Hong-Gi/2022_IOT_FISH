...
def main(_argv):
    ...
    while True:
        ...
        detected.append(len(detections))
        ...
        for track in tracker.tracks:
            ...
            if str(track.track_id) not in detectedIDs:
                detectedIDs.append(str(track.track_id))
            ...
            if old_detection != None:
                ...
                for oKey, oValue in old_detection.items():
                    ...
                    if abs(c_curr[0]-oValue[0]) + abs(c_curr[1]-oValue[1]) < 0:
    ...
    for key, values in fish_moving.items():
        if int(sum(values)/10000) == 0:
            continue
        if inputFlag == 0:
            fish01.append(round(sum(values)/10000, 1))
            inputFlag = 1
            continue
        elif inputFlag == 1:
            fish02.append(round(sum(values)/10000, 1))
            inputFlag = 0
            continue
                
    data01 = ( date, time_, 'fish01', 2, sum(fish01) )
    data02 = ( date, time_, 'fish02', 2, sum(fish02) )

    try:
        print(sum(fish01), sum(fish02))
        sql01 = "INSERT INTO mobility (Date, Time, obj_id, obj_num, mobility) VALUES(%s, %s, %s, %s, %s);"
        cur.execute(sql01, data01)

        sql02 = "INSERT INTO mobility (Date, Time, obj_id, obj_num, mobility) VALUES(%s, %s, %s, %s, %s);"
        cur.execute(sql02, data02)
